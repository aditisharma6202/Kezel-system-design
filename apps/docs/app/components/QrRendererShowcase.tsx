...
export default function QrRendererShowcase() {
  ...
  const controlLabelStyle: React.CSSProperties = { color: "var(--kz-component-input-text)" };
  const controlHintStyle: React.CSSProperties = { color: "var(--kz-component-input-placeholder)" };
  ...
  return (
    <section className="flex flex-col items-center gap-6 w-full max-w-3xl">
      ...
      <div className="w-full">
        <Typography variant={TypographyVariantEnum.H3}>Generate from Data</Typography>
        <Typography variant={TypographyVariantEnum.Caption}>
          Pass a <code>data</code> prop to generate a QR code on the fly with custom foreground/background colors.
        </Typography>
      </div>

      {/* Generate controls */}
      <div className="flex flex-wrap items-center gap-4 w-full">
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          Data
          <input
            type="text"
            value={genData}
        ...
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          ...
          <span className="font-mono text-xs" style={controlHintStyle}>{fgColor}</span>
        </label>
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          BG
          <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} />
          <span className="font-mono text-xs" style={controlHintStyle}>{bgColor}</span>
        </label>
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          Size
          <input
            ...
            className="w-20"
          />
          <span className="font-mono text-xs" style={controlHintStyle}>{genSize}px</span>
        </label>
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          ECL
          <select
      ...
      <div className="w-full mt-4">
        <Typography variant={TypographyVariantEnum.H3}>Pre-rendered Image</Typography>
        <Typography variant={TypographyVariantEnum.Caption}>
          Pass an <code>image</code> prop with a base64 string, data URI, or URL.
        </Typography>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 w-full">
        <label className="flex items-center gap-2 text-sm" style={controlLabelStyle}>
          ...
          <input
            ...
            className="w-28"
          />
          <span className="font-mono text-xs" style={controlHintStyle}>{size}px</span>
        </label>
        <label className="flex items-center gap-2 text-sm cursor-pointer" style={controlLabelStyle}>
