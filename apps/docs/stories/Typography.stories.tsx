...
export const AllVariants: Story = {
  render: () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 560 }}>
      <Typography variant={TypographyVariantEnum.H1}>Heading 1 — 24px</Typography>
      <Typography variant={TypographyVariantEnum.H2}>Heading 2 — 24px</Typography>
      <Typography variant={TypographyVariantEnum.H3}>Heading 3 — 24px</Typography>
...
const TYPOGRAPHY_TOKEN_KEYS = [
  { key: "typography.error.color", cssVar: "--kz-typography-error-color", example: "#ef4444" },
  { key: "typography.success.color", cssVar: "--kz-typography-success-color", example: "#22c55e" },
  { key: "typography.warning.color", cssVar: "--kz-typography-warning-color", example: "#f59e0b" },
] as const;

export const TypographyTokenOverrides: Story = {
  render: () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 32, padding: 24, maxWidth: 720 }}>
      <div>
        <h3 style={{ marginTop: 0, marginBottom: 8 }}>Tokens to override typography colors</h3>
        <p style={{ color: "#52606d", fontSize: 14, marginBottom: 16 }}>
          Pass these keys in the <code>tokens</code> prop of <code>KezelThemeProvider</code> to
          override error, success, and warning typography colors. Values must be valid CSS colors
          (hex, rgb, hsl, etc.).
        </p>
        <table
          style={{
        ...
        >
          <thead>
            <tr style={{ borderBottom: "1px solid #e4e7eb", textAlign: "left" }}>
              <th style={{ padding: "8px 12px", fontWeight: 600 }}>Token key (for <code>tokens</code>)</th>
              <th style={{ padding: "8px 12px", fontWeight: 600 }}>CSS variable</th>
              <th style={{ padding: "8px 12px", fontWeight: 600 }}>Example value</th>
            </tr>
          </thead>
          <tbody>
            {TYPOGRAPHY_TOKEN_KEYS.map(({ key, cssVar, example }) => (
              <tr key={key} style={{ borderBottom: "1px solid #f0f2f5" }}>
                <td style={{ padding: "8px 12px", fontFamily: "monospace", fontSize: 12 }}>{key}</td>
      ...
      <div>
        <h3 style={{ marginBottom: 12 }}>Example: custom typography colors via tokens</h3>
        <KezelThemeProvider
        ...
        >
          ...
          >
            ...
            <Typography variant={TypographyVariantEnum.Success} style={{ marginTop: 8 }}>
              Success message (custom green)
            </Typography>
            <Typography variant={TypographyVariantEnum.Warning} style={{ marginTop: 8 }}>
...
export const WithTone: Story = {
  render: () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <Typography variant={TypographyVariantEnum.Body} tone={TypographyToneEnum.Primary}>
        Primary tone
      </Typography>
      <Typography variant={TypographyVariantEnum.Body} tone={TypographyToneEnum.Secondary}>
        Secondary tone
      </Typography>
      <Typography variant={TypographyVariantEnum.Body} tone={TypographyToneEnum.Muted}>
        Muted tone
      </Typography>
      <Typography variant={TypographyVariantEnum.Body} tone={TypographyToneEnum.Link} as="a" href="#">
        Link tone (as anchor)
      </Typography>
    </div>
  ),
...
export const WithHref: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Pass `href` to render as an anchor. When `href` is set, the component renders as <a> regardless of `as`.",
      },
    },
  },
};
...
export const Alignment: Story = {
  render: () => (
    <div style={{ width: 320, border: "1px dashed #e4e7eb", padding: 8 }}>
      <Typography variant={TypographyVariantEnum.Body} align={TypographyAlignEnum.Left}>
        Align left
      </Typography>
      <Typography variant={TypographyVariantEnum.Body} align={TypographyAlignEnum.Center}>
        Align center
      </Typography>
      <Typography variant={TypographyVariantEnum.Body} align={TypographyAlignEnum.Right}>
        Align right
      </Typography>
    </div>
  ),
