import React from "react";
import type { Meta, StoryObj } from "@storybook/react";
import {
  QrRenderer,
  QrPattern,
  Button,
  ButtonVariant,
  ButtonSize,
} from "kz-design-system";

const meta: Meta<typeof QrRenderer> = {
  title: "Design System/QrRenderer",
  component: QrRenderer,
  parameters: {
    layout: "centered",
    docs: {
      description: {
        component:
          "Renders QR codes from pre-built images or generates them from text/URL data with custom foreground/background colors and dot patterns (square, circle, diamond). Supports brand logo overlay, label, caption, debug info, download, and error correction levels. Adapts to all theme variants.",
      },
    },
  },
  tags: ["autodocs"],
  argTypes: {
    size: { control: { type: "range", min: 100, max: 400, step: 20 } },
    foregroundColor: { control: "color" },
    backgroundColor: { control: "color" },
    downloadable: { control: "boolean" },
    showDebug: { control: "boolean" },
    downloadFilename: { control: "text" },
    alt: { control: "text" },
    pattern: {
      control: "select",
      options: [QrPattern.Square, QrPattern.Circle, QrPattern.Diamond],
    },
  },
};

export default meta;

type Story = StoryObj<typeof QrRenderer>;

/* ── Generate from data ── */

export const Default: Story = {
  args: {
    data: "https://kezel.dev",
    size: 240,
    foregroundColor: "#000000",
    backgroundColor: "#ffffff",
    label: "Scan to visit",
    caption: "kezel.dev",
    downloadable: true,
  },
  parameters: {
    docs: {
      description: {
        story:
          "Generate a QR code from a `data` string. Use the controls to change foreground/background colors, size, pattern, and more.",
      },
    },
  },
};

/* ── Pattern styles ── */

export const PatternStyles: Story = {
  render: () => (
    <div
      style={{
        display: "flex",
        gap: 24,
        alignItems: "start",
        flexWrap: "wrap",
      }}
    >
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        pattern={QrPattern.Square}
        label="Square"
        caption="Default pattern"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        pattern={QrPattern.Circle}
        label="Circle"
        caption="Rounded dots"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        pattern={QrPattern.Diamond}
        label="Diamond"
        caption="Diamond shaped"
      />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story:
          "Three dot pattern styles: **Square** (default), **Circle**, and **Diamond**. Use the `pattern` prop with the `QrPattern` enum.",
      },
    },
  },
};

/* ── With brand logo ── */

export const WithBrandLogo: Story = {
  render: () => (
    <div
      style={{
        display: "flex",
        gap: 24,
        alignItems: "start",
        flexWrap: "wrap",
      }}
    >
      <QrRenderer
        data="https://kezel.dev"
        size={240}
        pattern={QrPattern.Circle}
        logo="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/120px-React-icon.svg.png"
        logoScale={0.22}
        label="Circle + Logo"
        caption="Brand logo in center"
        downloadable
      />
      <QrRenderer
        data="https://kezel.dev"
        size={240}
        pattern={QrPattern.Diamond}
        logo="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/120px-React-icon.svg.png"
        logoScale={0.22}
        label="Diamond + Logo"
        caption="Brand logo in center"
        downloadable
      />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story:
          "Pass a **logo** prop (URL, data URI, or base64) to overlay a brand logo in the center. Error correction is automatically raised to **H** when a logo is present. Adjust logo size with **logoScale** (fraction of QR size).",
      },
    },
  },
};

export const CustomColors: Story = {
  render: () => (
    <div
      style={{
        display: "flex",
        gap: 24,
        alignItems: "start",
        flexWrap: "wrap",
      }}
    >
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        foregroundColor="#1a1a2e"
        backgroundColor="#e2e8f0"
        pattern={QrPattern.Circle}
        label="Dark blue on slate"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        foregroundColor="#166534"
        backgroundColor="#f0fdf4"
        pattern={QrPattern.Diamond}
        label="Green on mint"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={200}
        foregroundColor="#9f1239"
        backgroundColor="#fff1f2"
        label="Rose on pink"
      />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story:
          "Use **foregroundColor** and **backgroundColor** to brand QR codes with custom color schemes. Combine with different patterns for unique styles.",
      },
    },
  },
};

export const ErrorCorrectionLevels: Story = {
  render: () => (
    <div
      style={{
        display: "flex",
        gap: 24,
        alignItems: "start",
        flexWrap: "wrap",
      }}
    >
      <QrRenderer
        data="https://kezel.dev"
        size={180}
        errorCorrectionLevel="L"
        label="L (7%)"
        caption="Low"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={180}
        errorCorrectionLevel="M"
        label="M (15%)"
        caption="Medium"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={180}
        errorCorrectionLevel="Q"
        label="Q (25%)"
        caption="Quartile"
      />
      <QrRenderer
        data="https://kezel.dev"
        size={180}
        errorCorrectionLevel="H"
        label="H (30%)"
        caption="High"
      />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story:
          "Four error correction levels. Higher levels produce denser codes but allow more damage/obstruction (e.g. logo overlay). Use **H** when adding a logo.",
      },
    },
  },
};

export const Sizes: Story = {
  render: () => (
    <div
      style={{
        display: "flex",
        gap: 24,
        alignItems: "start",
        flexWrap: "wrap",
      }}
    >
      <QrRenderer data="https://kezel.dev" size={120} label="120px" />
      <QrRenderer data="https://kezel.dev" size={200} label="200px" />
      <QrRenderer data="https://kezel.dev" size={300} label="300px" />
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story: "Control the rendered size with the **size** prop (in pixels).",
      },
    },
  },
};

export const WithDownload: Story = {
  args: {
    data: "https://kezel.dev/download-demo",
    size: 240,
    downloadable: true,
    downloadFilename: "kezel-qr.png",
    label: "Downloadable",
    caption: "Click the button below to save",
  },
  parameters: {
    docs: {
      description: {
        story:
          "Set **downloadable** to show a download button. Customize the filename with **downloadFilename**.",
      },
    },
  },
};

/* ── Interactive playground story ── */

function QrPlayground() {
  const [data, setData] = React.useState("https://kezel.dev");
  const [fg, setFg] = React.useState("#000000");
  const [bg, setBg] = React.useState("#ffffff");
  const [size, setSize] = React.useState(240);
  const [pattern, setPattern] = React.useState(QrPattern.Square);
  const [logoUrl, setLogoUrl] = React.useState("");
  const [logoPreview, setLogoPreview] = React.useState<string | undefined>(
    undefined
  );
  const fileInputRef = React.useRef<HTMLInputElement>(null);

  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      const result = reader.result as string;
      setLogoPreview(result);
      setLogoUrl("");
    };
    reader.readAsDataURL(file);
  };

  const effectiveLogo = logoUrl || logoPreview || undefined;

  const controlStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    gap: 6,
    fontSize: 13,
    color: "var(--kz-component-input-text)",
  };

  const inputStyle: React.CSSProperties = {
    padding: "4px 8px",
    borderRadius: 6,
    border: "1px solid var(--kz-component-input-border)",
    background: "var(--kz-component-input-bg)",
    color: "var(--kz-component-input-text)",
    fontSize: 13,
  };

  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        gap: 16,
        alignItems: "center",
      }}
    >
      <div
        style={{
          display: "flex",
          gap: 12,
          flexWrap: "wrap",
          justifyContent: "center",
        }}
      >
        <label style={controlStyle}>
          Data
          <input
            type="text"
            value={data}
            onChange={(e) => setData(e.target.value)}
            style={{ ...inputStyle, width: 180 }}
          />
        </label>
        <label style={controlStyle}>
          FG{" "}
          <input
            type="color"
            value={fg}
            onChange={(e) => setFg(e.target.value)}
          />
        </label>
        <label style={controlStyle}>
          BG{" "}
          <input
            type="color"
            value={bg}
            onChange={(e) => setBg(e.target.value)}
          />
        </label>
        <label style={controlStyle}>
          Size
          <input
            type="range"
            min={100}
            max={400}
            step={20}
            value={size}
            onChange={(e) => setSize(Number(e.target.value))}
            style={{ width: 80 }}
          />
          <span
            style={{
              fontFamily: "monospace",
              fontSize: 11,
              color: "var(--kz-component-input-placeholder)",
            }}
          >
            {size}px
          </span>
        </label>
        <label style={controlStyle}>
          Pattern
          <select
            value={pattern}
            onChange={(e) => setPattern(e.target.value as QrPattern)}
            style={inputStyle}
          >
            <option value={QrPattern.Square}>Square</option>
            <option value={QrPattern.Circle}>Circle</option>
            <option value={QrPattern.Diamond}>Diamond</option>
          </select>
        </label>
      </div>
      <div
        style={{
          display: "flex",
          gap: 12,
          flexWrap: "wrap",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        <label style={controlStyle}>
          Logo URL
          <input
            type="text"
            value={logoUrl}
            onChange={(e) => {
              setLogoUrl(e.target.value);
              setLogoPreview(undefined);
            }}
            placeholder="https://..."
            style={{ ...inputStyle, width: 200 }}
          />
        </label>
        <div style={controlStyle}>
          or
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
            onChange={handleLogoUpload}
            style={{ display: "none" }}
          />
          <Button
            variant={ButtonVariant.Primary}
            size={ButtonSize.Sm}
            onClick={() => fileInputRef.current?.click()}
          >
            Upload Logo
          </Button>
          {effectiveLogo && (
            <span
              style={{
                fontSize: 11,
                color: "var(--kz-component-input-placeholder)",
              }}
            >
              Logo loaded
            </span>
          )}
        </div>
        {effectiveLogo && (
          <Button
            variant={ButtonVariant.Ghost}
            size={ButtonSize.Sm}
            onClick={() => {
              setLogoUrl("");
              setLogoPreview(undefined);
              if (fileInputRef.current) fileInputRef.current.value = "";
            }}
          >
            Clear logo
          </Button>
        )}
      </div>
      <QrRenderer
        data={data}
        size={size}
        foregroundColor={fg}
        backgroundColor={bg}
        pattern={pattern}
        logo={effectiveLogo}
        logoScale={0.22}
        label="Live Preview"
        caption={data}
        downloadable
      />
    </div>
  );
}

export const InteractivePlayground: Story = {
  render: () => <QrPlayground />,
  parameters: {
    docs: {
      description: {
        story:
          "Interactive demo with color pickers, pattern selector, and brand logo upload. Upload your own logo or paste a URL to see it centered on the QR code.",
      },
    },
  },
};

/* ── Pre-rendered image mode ── */

const SAMPLE_QR_BASE64 =
  "";

const SAMPLE_DEBUG = {
  qr_size_modules: 37,
  module_size_px: 8,
  native_img_px: 296,
  out_size_px: 320,
  pattern_style: "circle" as const,
  border_modules: 2,
  org: "LGw1YdzDRPZR",
  has_logo: true,
  logo_source: "file",
};

export const PreRenderedImage: Story = {
  args: {
    image: SAMPLE_QR_BASE64,
    size: 240,
    label: "From base64",
    caption: "Pre-rendered image",
    downloadable: true,
  },
  parameters: {
    docs: {
      description: {
        story:
          "Pass a pre-rendered QR code via the **image** prop (base64, data URI, or URL). Colors cannot be changed in this mode.",
      },
    },
  },
};

export const WithDebugInfo: Story = {
  args: {
    image: SAMPLE_QR_BASE64,
    size: 240,
    label: "Debug mode",
    showDebug: true,
    debug: SAMPLE_DEBUG,
  },
  parameters: {
    docs: {
      description: {
        story:
          "Set **showDebug** and pass a **debug** object to display QR metadata below the code.",
      },
    },
  },
};
